...
def clean_product_name(name):
    ...
    # 괄호 안 변종/색상/성분 옵션 리스트 제거 (퍼플/그린), (레티놀, 비타민C), (마데카소사이드, PDRN, 히알루론산)
    s = re.sub(r'\s*\([^)]*[/,][^)]*\)\s*$', '', s)
    # 특정 슬래시 패턴 먼저 제거 (제품명 보존)
    s = re.sub(r'/본품\+리필.*$', '', s)  # 쿠션/본품+리필 → 쿠션
    s = re.sub(r'\s+단품/기획$', '', s)
    s = re.sub(r'\s+단품/더블기획$', '', s)
    s = re.sub(r'\s+\(단품/기획\)$', '', s)
    # 슬래시 구분 옵션 제거 (위에서 안 잡힌 나머지)
    s = re.sub(r'\s+(?:\S+/){2,}\S+', '', s)  # 3개 이상 슬래시 구분
    ...
    s = re.sub(r'\s*\d+(?:\.\d+)?(?:ml|ML|mL|g|G|mg|매|입|ea|회분)\b', '', s)
    ...
    # "고기능" 등 골라담기 잔여 키워드
    s = re.sub(r'\s+고기능$', '', s)
    # 기획/프로모션 키워드 (띄어쓰기 있는 "더블 기획" 포함)
    s = re.sub(r'\s+(?:더블|듀오|대용량|리필)\s+기획(?:세트)?(?:\s.*)?$', '', s)
    ...
    # 증정 정보
    s = re.sub(r'\s*\+.*증정.*$', '', s)
    # 빈 괄호 제거 (용량 제거 후 남은 잔여물)
    s = re.sub(r'\s*\([^a-zA-Z가-힣]*\)', '', s)
