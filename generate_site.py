...
def build_keywords_html(data):
    ...
    if nv_rising:
        ...
        for i, kw in enumerate(nv_rising[:10], 1):
            ...
            display_kw = kw.get("keyword_en") or kw.get("keyword", "")
  ...
  <span class="kw-rate">+{kw["change_rate"]:.0f}</span>
    ...
    if yt_rising:
        ...
        for i, kw in enumerate(yt_rising[:10], 1):
            w = min(100, (kw["change_rate"] / mx) * 100) if mx > 0 else 0
            items += f'''<div class="kw-item">
  <span class="kw-rank">#{i}</span><span class="kw-text">{esc(kw.get("keyword_en") or kw.get("keyword", ""))}</span>
...
def build_seller_html(data):
  ...
  <p class="ss-teaser">&#128274; ดูรายชื่อทั้งหมดได้ที่แท็บ <a href="#" onclick="document.querySelectorAll('.tab-btn')[3].click();return false" style="color:#e8547a;font-weight:700">รายงาน Pro</a></p>
    ...
    if hgs:
        ...
        for hg in hgs:
            ...
            items += f'''<div class="si si-gem"><div class="si-name">{esc(en_name)}</div>
  ...
  <p class="ss-desc">สินค้าขายดีในเกาหลี แต่ยังไม่เป็นกระแสในโซเชียล</p>{items}</div>'''
    ...
    if sss:
        ...
        for ss in sss:
            ...
            items += f'''<div class="si si-steady"><div class="si-name">{esc(en_name)}</div>
  ...
  <div class="si-name">{esc(en_name)}</div>
</div>'''
...
.about-link{{display:inline-block;margin-top:10px;font-size:12px;color:#fff;text-decoration:none;background:rgba(255,255,255,.2);padding:5px 16px;border-radius:16px;border:1px solid rgba(255,255,255,.5);font-weight:600;letter-spacing:.3px}}.about-link:hover{{background:rgba(255,255,255,.35);border-color:#fff}}
...
.btn-buy{{display:block;margin-top:3px;padding:4px 10px;background:#e8547a;color:#fff;border:none;border-radius:6px;font-size:10px;font-weight:600;cursor:pointer;font-family:inherit;line-height:1.3;text-align:center}}
...
def main():
    ...
    # 영어 버전 생성
    html_en = make_english_version(html_th)
    out_en = os.path.join(DOCS_DIR, "en.html")
