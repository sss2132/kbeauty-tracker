## ⛔ 실행 전 승인 필수
파이프라인의 각 Step(step1~step5)을 실행하기 전에 반드시 사용자 승인을 받는다. 별도 agent(claude -p, Agent tool 등)의 작업도 사용자 승인 없이 직접 실행하지 않는다. 승인 없이 자의적으로 실행하거나, 기존 프로세스를 우회하여 대체 실행하는 것을 금지한다.
...
## 세션 구조 (Remote Control)
- 매일 19:00 Windows 스케줄러가 `scheduled_daily_collect.ps1` 실행
- 텔레그램 API로 알림 전송 + `claude --remote-control "K-Beauty Daily"` 세션 시작
- 사용자는 Claude 모바일앱/웹(claude.ai/code) 또는 로컬 터미널에서 접속
- 수동 트리거: `schtasks /run /tn KBeauty-DailyCollect`

## 자료 수집 명령
"시작", "자료 수집 시작", "수집 시작" 등의 메시지가 오면 kbeauty-tracker/run_daily_collect.py 파이프라인을 전체 실행한다.
...
- Step 3에서 `verify_english_names()`가 needs_confirm을 반환하면:
  - 반드시 글로벌몰/공식몰 검색 결과를 포함하여 사용자에게 확인 요청
  - 사용자가 승인하면:
...
### 알림 규칙
- 각 Step 시작/완료 시 진행 상황을 세션에 출력
- 에러, 타임아웃, 검증 실패 등 문제 발생 시 즉시 사용자에게 알리고 지시를 기다림 (자의적으로 건너뛰지 않음)
- 전체 완료 후 결과 요약을 출력
